...
interface RelacionElemento {
  id: string;
  tipo: 'agua_cultivo' | 'proveedor_insumo' | 'cliente_producto' | 'trabajador_cultivo';
  elemento_origen: string; // ID del elemento que provee
  elemento_destino: string; // ID del elemento que recibe
  descripcion: string;
  dependencia_critica: boolean; // Si es crítica para la operación
  capacidad_servicio?: number; // Ej: litros/día para agua
  porcentaje_cobertura?: number; // 0-100% de cobertura
}

interface GestionRelaciones {
  relaciones: RelacionElemento[];
  // Ejemplo: reservorio_1 sirve a cultivo_1 (80%), cultivo_2 (20%)
  // proveedor_1 suministra fertilizantes a cultivo_1, cultivo_3
}

// 📍 INTERFAZ COMÚN DE UBICACIÓN SIMPLIFICADA PARA IA
...
// 📦 DATOS ESENCIALES SIMPLIFICADOS PARA IA
interface MovilidadLogistica {
  transporte_principal: 'vehiculo_propio' | 'contratado' | 'cooperativa' | 'publico';
  capacidad_carga_kg: number;
  costo_transporte_mes: number;
}
...
interface Cultivo {
  ...
  etapaCrecimiento: 'semilla' | 'crecimiento' | 'floracion' | 'cosecha';
  riesgClimatico: number; // 0-100
  rendimientoEstimado: number; // kg/ha
  posicion: { fila: number; columna: number };
  // 🤖 DATOS PARA IA
  ubicacion: UbicacionGeodata;
  dependencias_agua: string[]; // IDs de reservorios que lo sirven
  relacion_elementos: string[]; // IDs de elementos relacionados
}
...
interface Proveedor {
  ...
  // 🤖 DATOS PARA IA
  ubicacion: UbicacionGeodata;
  zonas_cobertura: string[]; // Departamentos que atiende
  productos_principales: string[]; // Lista de productos clave
}

interface Cliente {
  id: string;
  nombre: string;
  tipo: 'mayorista' | 'minorista' | 'mercado_local' | 'exportacion';
  contacto: { telefono: string; email: string; };
  volumenCompra: number; // kg/mes promedio
  precioPromedio: number; // precio por kg
  posicion: { fila: number; columna: number };
  // 🤖 DATOS PARA IA
  ubicacion: UbicacionGeodata;
  productos_demandados: string[]; // Tipos de cultivos que compra
  relacion_elementos: string[]; // IDs de cultivos que le suministramos
}

interface Trabajador {
  id: string;
  nombre: string;
  cargo: 'jornalero' | 'especialista' | 'supervisor' | 'administrador';
  experiencia: number; // años
  salario: number;
  telefono: string;  posicion: { fila: number; columna: number };
  // 🤖 DATOS PARA IA
  ubicacion: UbicacionGeodata;
  especialidades: string[]; // Cultivos o tareas especializadas
  disponibilidad: 'completa' | 'parcial' | 'temporal';
  cultivos_asignados: string[]; // IDs de cultivos bajo su responsabilidad
}

interface Cliente {
  id: string;
  nombre: string;
  tipo: 'individual' | 'empresa' | 'cooperativa';
  contacto: {
    telefono: string;
    email: string;
    direccion: string;
...
// 🏪 FORMULARIO INTELIGENTE PARA PROVEEDORES SIMPLIFICADO CON MAPSELECTOR
const FormularioProveedorInteligente: React.FC<{ 
  formData: any; 
  setFormData: (data: any) => void; 
  ubicacionesExistentes: UbicacionGeodata[];
  elementos?: any[] // Para mostrar relaciones
}> = ({ formData, setFormData, ubicacionesExistentes = [], elementos = [] }) => {
  const [pasoActual, setPasoActual] = useState(1);
  const totalPasos = 3; // Simplificado a 3 pasos esenciales
  const [mostrarMapSelector, setMostrarMapSelector] = useState(false);

  const handleUbicacionSeleccionada = (locationData: any) => {
    const ubicacion: UbicacionGeodata = {
      departamento: locationData.department || 'No especificado',
      provincia: locationData.province || 'No especificado', 
      distrito: locationData.district || 'No especificado',
      coordenadas: { lat: locationData.lat, lng: locationData.lng },
      altitud: locationData.altitude,
      accesibilidad: 'buena', // Valor por defecto
      distancia_capital_km: 0 // Se calculará después
    };
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-bold text-orange-700 mb-2">🏪 Datos Básicos del Proveedor</h3>
              <p className="text-gray-600">Información esencial para análisis de IA</p>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-orange-50 rounded-lg">
                <Label htmlFor="nombre" className="font-semibold text-orange-800">Nombre del Proveedor *</Label>
                <Input
                  id="nombre"
                  value={formData.nombre || ''}
                  onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                <Label htmlFor="tipo" className="font-semibold text-blue-800">Especialidad *</Label>
                <Select value={formData.tipo || ''} onValueChange={(value) => setFormData({ ...formData, tipo: value })}>
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                <Label className="font-semibold text-green-800">Contacto</Label>
              ...
              <div className="p-4 bg-purple-50 rounded-lg">
                <Label className="font-semibold text-purple-800">Calidad & Confianza</Label>
                <div className="space-y-2 mt-2">
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-bold text-blue-700 mb-2">📍 Ubicación Geográfica</h3>
              <p className="text-gray-600">Determina la zona de cobertura del proveedor</p>
            </div>

            <div className="space-y-4">
              <Button 
                onClick={() => setMostrarMapSelector(true)}
                className="w-full h-16 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700"
              >
                <MapPin className="h-6 w-6 mr-3" />
                {formData.ubicacion ? 'Cambiar Ubicación en Mapa' : 'Seleccionar Ubicación en Mapa'}
              </Button>

              {formData.ubicacion && (
                <div className="p-4 bg-blue-50 rounded-lg">
                  <h4 className="font-semibold text-blue-800 mb-2">📍 Ubicación Seleccionada</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                    <div>
                      <strong>Departamento:</strong> {formData.ubicacion.departamento}
                    </div>
                    <div>
                      <strong>Provincia:</strong> {formData.ubicacion.provincia}
                    </div>
                    <div>
                      <strong>Distrito:</strong> {formData.ubicacion.distrito}
                    </div>
                  </div>
                  {formData.ubicacion.coordenadas && (
                    <div className="mt-2 text-xs text-gray-600">
                      Coordenadas: {formData.ubicacion.coordenadas.lat.toFixed(4)}, {formData.ubicacion.coordenadas.lng.toFixed(4)}
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="p-4 bg-green-50 rounded-lg">
                <Label className="font-semibold text-green-800">Cobertura de Servicio</Label>
                <div className="space-y-2 mt-2">
                  <Select value={formData.accesibilidad || ''} onValueChange={(value) => setFormData({ ...formData, accesibilidad: value })}>
                    <SelectTrigger>
                      <SelectValue placeholder="Accesibilidad de la zona" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="excelente">🟢 Excelente - Acceso directo</SelectItem>
                      <SelectItem value="buena">🔵 Buena - Carretera asfaltada</SelectItem>
                      <SelectItem value="regular">🟡 Regular - Carretera afirmada</SelectItem>
                      <SelectItem value="dificil">🔴 Difícil - Solo trocha</SelectItem>
                    </SelectContent>
                  </Select>
                  <Input
                    type="number"
                    placeholder="Distancia a capital de provincia (km)"
                    value={formData.distancia_capital_km || ''}
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6">
              ...
              <h3 className="text-xl font-bold text-green-700 mb-2">🔗 Relaciones y Análisis IA</h3>
              <p className="text-gray-600">Conexiones con cultivos y análisis inteligente</p>
            </div>

            <div className="space-y-4">
              {/* Relaciones con cultivos */}
              <div className="p-4 bg-green-50 rounded-lg">
                <Label className="font-semibold text-green-800">🌱 Cultivos que Atiende</Label>
                <div className="mt-2 space-y-2">
                  {['papa', 'quinua', 'maiz', 'habas', 'oca', 'ulluco'].map(cultivo => (
                    <label key={cultivo} className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        checked={formData.productos_principales?.includes(cultivo) || false}
                        onChange={(e) => {
                          const productos = formData.productos_principales || [];
                          if (e.target.checked) {
                            setFormData({ ...formData, productos_principales: [...productos, cultivo] });
                          } else {
                            setFormData({ ...formData, productos_principales: productos.filter((p: string) => p !== cultivo) });
                          }
                        }}
                        className="rounded"
                      />
                      <span className="text-sm capitalize">{cultivo}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 bg-blue-50 rounded-lg">
                <h4 className="font-semibold text-blue-800 mb-3">🤖 Análisis Inteligente</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>Confiabilidad:</span>
                    <Badge variant="outline">
                      {formData.confiabilidad ? `${formData.confiabilidad}/5 ⭐` : 'No calificado'}
                    </Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>Especialidad:</span>
                    <Badge variant="secondary">{formData.tipo || 'No especificado'}</Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>Ubicación:</span>
                    <Badge variant="outline">
                      {formData.ubicacion?.departamento || 'No definida'}
                    </Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>Productos:</span>
                    <Badge variant="secondary">
                      {formData.productos_principales?.length || 0} cultivos
                    </Badge>
                  </div>
                </div>
              </div>
