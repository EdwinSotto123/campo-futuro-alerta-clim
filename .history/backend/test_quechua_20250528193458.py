...
def test_quechua_prompt():
    ...
    quechua_indicators = ["QUECHUALLAPI", "RIMAYLLAM", "kanki", "chakra", "yanapakuq"]
    indicators_found = [indicator for indicator in quechua_indicators if indicator in quechua_prompt]
...
def test_quechua_ai_response():
    ...
    try:
        ...
        # Verificar que el prompt del sistema esté en Quechua
        ...
        Perú suyupi papa tarpuykunamanta yachayta munani. Kay tapukuykunata kutichiy:
        
        1. ¿Imayna ruwana papa tarpuy sumaq kanampaq?
        2. ¿Ima pachapi papa tarpuna allin?
        3. ¿Ima unquykunatan papa hapisqa kanman?
        4. ¿Imayna allpata wakichanam?
        5. ¿Imayna yakuwan qhuyay?
        
        IMPORTANTE: Tukuy kutichiykunata quechuallapi qillqay. Ama español rimaykuchu!
        Chakra llamkaqkunapaq allin yachaykunata quy.
        """
        ...
        quechua_words = ["papa", "tarpuy", "chakra", "allpa", "yaku", "inti", "killa", 
                        "wata", "sumaq", "allin", "ruwana", "pacha", "unquy"]
        ...
        spanish_words = ["agricultura", "cultivo", "fertilizante", "planta", "semilla", 
                        "cosecha", "riego", "suelo"]
        spanish_found = [word for word in spanish_words if word.lower() in quechua_response.lower()]
        
        print(f"🔍 Análisis de idioma:")
        print(f"   Palabras Quechua detectadas: {words_found}")
        print(f"   Palabras Español detectadas: {spanish_found}")
        
        if len(words_found) >= 5 and len(spanish_found) <= 2:
            print(f"✅ Respuesta confirmada en Quechua")
            return True
        elif len(words_found) >= 3:
            print(f"⚠️ Respuesta parcialmente en Quechua")
            return False
        else:
            print("❌ La respuesta NO está en Quechua")
            return False
